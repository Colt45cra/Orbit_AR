...
internal class AimStability(private val holdNanos: Long = 350_000_000L) {
    ...
    fun reset() { surface=null; origin=null; since=0 }
    fun update(id: Any?, position: Point3?, timestamp: Long): Boolean {
        ...
        if(surface!=id || moved || timestamp<since) { surface=id; origin=position; since=timestamp; return false }
        return timestamp-since >= holdNanos
    }
}
