...
@Composable
internal fun SceneScope.TriggerContent(bitmap:Bitmap,width:Float,tilt:Float,offsetX:Float,offsetZ:Float) {
    val aspect=bitmap.width.toFloat()/bitmap.height.coerceAtLeast(1)
    val height=width/aspect
    val center=placementCenter(height,tilt,offsetX,offsetZ)
    Node(position=Position(center.x,center.y,center.z),rotation=Rotation(x=tilt-90f),scale=Scale(width)) {
